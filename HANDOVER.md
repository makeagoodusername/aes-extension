...
| Q11 | **Single-step undo** | Last write (override save, calibration, override clear, watchlist toggle) gets a 10-second toast with **Undo**. Cheap because every write goes through known store classes. | Cancel a fat-fingered override without re-typing values. Universal feature; we don't have it. | ~100 LOC |
| Q12 | **Persistent free-text search** | Search box in the toolbar that filters rows by IATA / city-name substring across every visible column. Survives across renders. | Faster than scroll-to-find on a wide-route hub (200+ destinations). | ~60 LOC |
...
| # | Slice | What | Why it's practical | Est |
|---|---|---|---|---|
| U1 | **Sticky table header** | Header row stays anchored as the body scrolls. CSS `position: sticky` on `<thead>`. | Required for any wide-row table; rule of thumb. | ~30 LOC |
...
| U14 | **Toast notifications** | Save / calibrate / sync confirmations as bottom-right toasts (3-second auto-dismiss, click to navigate to the affected row). Replaces console-only confirmation. | Today there's no visual feedback on save — users wonder "did it take?" | ~80 LOC |
| U15 | **Group-header hover highlight** | Hovering a group header tints the column band so the user sees "ah, those four columns are all 'markets'". Pure CSS. | Cheap discoverability win — group structure is hard to see in a wide table. | ~20 LOC |
