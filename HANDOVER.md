...
| # | Slice | What | Why it's practical | Est |
|---|---|---|---|---|
| Q1 | **Quick-filter chips bar** | One-click toggle chips above the table — `[Watchlist] [NEW] [Loss-makers] [Demand≥8] [No override] [Aircraft fits]`. Reuses the existing filter store, just adds a fast path that doesn't open the drawer. | Saves clicks for the 80% case (filtering by status / watchlist / loss-makers is the daily-driver workflow). | ~80 LOC |
...
| Q15 | **Hub keyboard quick-swap** | `⌘1 / ⌘2 / ⌘3` shortcuts navigate between your three most-active hubs without leaving the panel. Pulls top hubs from the schedule cache by route count. | Multi-hub navigation today requires a full URL change + reload. | ~50 LOC |
