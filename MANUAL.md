...
| **H** | Wave + interlining + slot management workspace | ✅ **Slices 1 + 2 shipped.** Slice 1: read-only Gantt with hour-ruler swim lanes per wave. Slice 2: in-panel preset editor (composition spinners, time inputs, add/remove waves, CRUD strip), drag-edit slot overrides via `wave-overrides-store.js`, connection-graph SVG with own/interline/alliance classifier, and explicit `💾 Save schedule` CTA persisting builds via `ScheduleStore.save` (with Undo). **Slice 3+ open:** `interline-store.js`, `slot-store.js`, LP/heuristic auto-optimise (replacing greedy fill), multi-hub network view. See HANDOVER §9 row "H slice 3+". |
...
| ~~**H slices 1 + 2 — Wave overlay foundation + drag-edit + connections + save**~~ | 3 | ~1300 LOC | ✅ **Shipped.** See §16 status overview row for the breakdown. |
| **H slice 3+ — interline / slot / auto-optimise / multi-hub** | 3 | ~600 LOC | OPEN. Highest-impact next item is auto-optimise (LP/heuristic placement maximising the connection graph) — the greedy `assignRoutes` leaves obvious connection gains on the table. |
