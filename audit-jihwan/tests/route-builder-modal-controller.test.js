...
await it("modal loads and exposes _internal helpers", async () => {
    ...
    for (const fn of ["parseIataList", "materialiseLegs", "buildApplyPayload",
                       ...
                       "mergeCandidateSources", "scheduledInfoFromSchedule",
                       "candidateSortScore"]) {
...
await it("defaultConfig defaults are sane; singleDayMask is 7-element 0/1", async () => {
    ...
    assert.strictEqual(cfg.hideScheduled, true)
...
// ── 3b) Route Assistant + schedule-store candidate merge ─────────────
await it("mergeCandidateSources joins AS in-game route intel and flags scheduled destinations", async () => {
    reset(); loadStack()
    const I = window.AesAfpRouteBuilderModal._internal
    const scheduled = I.scheduledInfoFromSchedule({
        scrapedAt: Date.now(),
        legs: [
            {origin: "JFK", destination: "ORD", flightId: "101"},
            {origin: "ORD", destination: "JFK", flightId: "102"}
        ]
    }, "JFK")
    assert.ok(scheduled.destSet.has("ORD"), "ORD should be detected from the in-game schedule")
    assert.strictEqual(scheduled.counts.get("ORD"), 2)

    const merged = I.mergeCandidateSources([
        {label: "FlightsFrom", routes: [
            {destIata: "ORD", destName: "Chicago", distanceKm: 1187, weeklyFlights: 77},
            {destIata: "MCO", destName: "Orlando", distanceKm: 1520, weeklyFlights: 119}
        ]},
        {label: "AS in-game", routes: [
            {destIata: "MCO", paxScore: 10, cargoScore: 5, score: 75,
             demandSource: "Route Assistant top routes"},
            {destIata: "BOS", distanceKm: 299, paxScore: 10, score: 73,
             source: "pricing cache"}
        ]}
    ], {
        hub: "JFK",
        scheduledDestSet: scheduled.destSet,
        scheduledCounts: scheduled.counts
    })

    const byDest = new Map(merged.map(c => [c.destIata, c]))
    assert.ok(byDest.has("MCO"), "MCO candidate missing")
    assert.ok(byDest.get("MCO").sources.includes("FlightsFrom"))
    assert.ok(byDest.get("MCO").sources.includes("Route Assistant top routes"))
    assert.strictEqual(byDest.get("MCO").paxScore, 10)
    assert.strictEqual(byDest.get("ORD").alreadyScheduled, true)
    assert.strictEqual(byDest.get("ORD").scheduledFlights, 2)
    assert.notStrictEqual(merged[0].destIata, "ORD",
        "already-scheduled candidates should sort behind open candidates")
})
...
// ── 9) Route Assistant in-game fields project onto Route Builder candidates ─
await it("projects schedule, market-analysis, and actuals onto route-builder candidates", async () => {
    reset(); loadStack()
    const I = window.AesAfpRouteBuilderModal._internal
    const row = {
        destIata: "JFK",
        sources: ["Route Assistant top routes"],
        paxScore: 7,
        cargoScore: 4,
        scoreBlend: 70
    }

    I.projectScheduleRecord(row, {
        weeklyFlights: 14,
        daysPerWeek: 7,
        departureTime: "06:00",
        ...
        cruiseSpeedKmh: 820,
        scrapedAt: 1777777777777
    })
    I.projectMarketsBucket(row, {
        ownPricing: {
            prices: {Y: 313, C: 620, Cargo: 1.25},
            defaults: {Y: 300, C: 600, Cargo: 1.1},
            generalSettings: {serviceProfile: "full service"}
        },
        marketShare: {
            period: "19/2026",
            pax: [
                {rank: 1, enterpriseId: 775, name: "CFLAIR", sharePct: 63.6, change: 40.1},
                {rank: 2, enterpriseId: 200, name: "Other Air", sharePct: 31.5, change: 132.3}
            ],
            cargo: [
                {rank: 1, enterpriseId: 200, name: "Other Air", sharePct: 24.5, change: 227}
            ]
        },
        competitors: {
            competitors: [
                {flightCode: "OA 123", serviceClass: "Y", price: 180, typeCode: "737", isOurs: false},
                {flightCode: "OA 124", serviceClass: "Y", price: 220, typeCode: "737", isOurs: false},
                {flightCode: "CF 99", serviceClass: "Y", price: 313, isOurs: true}
            ]
        }
    }, new Set([775]), "cflair")
    I.projectYieldHistoryRecord(row, {
        snapshots: [{timestamp: 1777777777000, profitPerFlight: 12000, profitPerWeek: 168000, frequency: 14}],
        lastSnapshotAt: 1777777777000
    })

    assert.strictEqual(row.liveWeeklyFlights, 14)
    assert.strictEqual(row.liveDeparture, "06:00")
    assert.strictEqual(row.liveAircraftType, "Boeing 737-700")
    assert.deepStrictEqual(row.ownPricing, {Y: 313, C: 620, Cargo: 1.25})
    assert.strictEqual(row.ourPaxShare, 63.6)
    assert.strictEqual(row.marketSharePeriod, "19/2026")
    assert.strictEqual(row.competitorCount, 1)
    assert.strictEqual(row.competitorMedianPriceY, 200)
    assert.strictEqual(row.actualProfitPerFlight, 12000)
    assert.strictEqual(row.actualProfitPerWeek, 168000)

    const summary = I.summariseCandidateSources([row], {destSet: new Set(["JFK"]), flightCount: 14, source: "aircraft-flights"})
    assert.strictEqual(summary.inGameRoutes, 1)
    assert.strictEqual(summary.priceRoutes, 1)
    assert.strictEqual(summary.marketRoutes, 1)
    assert.strictEqual(summary.actualRoutes, 1)
})

// ── 10) Tile module loads cleanly ─────────────────────────────────────
