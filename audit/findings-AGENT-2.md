...
## [FIXED] Cmp pill hover crash + per-competitor stats line (2026-05-03)

**Bug.** Hovering the **Cmp** pill in the Route Assistant table crashed the
panel ("competition menu" in the user's words). The pill rendered fine, but
the 200ms `mouseenter` → `_openCarrierPopover` path had no top-level
try/catch, so any throw inside the popover builder bubbled through and
froze the table.

**Defensive guards added** in `modules/route-assistant/panel.js`:

1. The Cmp column's intensity computation (`render(td, row)` for
   `airlineCount`) now guards `RouteAssistantCarriersScraper` with
   `typeof !== "undefined"`, mirroring the pattern the popover-builder at
   `_openCarrierPopover` already uses (lines ~20684–20689). Falls back to a
   gray pill instead of throwing if the scraper module isn't loaded.
2. Both popover-open call sites (Cmp column + Mkt% column) wrap the
   `_openCarrierPopover` invocation in try/catch with `console.error`.
   A future regression downstream now degrades to a no-op + console error
   rather than freezing the table.

**Stats-line enhancement.** Per-competitor row in the popover (built by
`_buildCarrierRow`) now renders a small line below the airline name:

```
[IATA] · ★rating · N ac · M emp · K hubs · {alliance name}
```

Sourced from a new `_applyCachedCompetitorIntel()` boot-time hydration that
reads the existing `competitorIntel:enterprise:<server>:<id>` and
`competitorIntel:alliance:<server>:<id>` keys directly via
`chrome.storage.local.get(null)`. Each segment is conditional — partial
records still render, and a row with no intel cached emits no line at all.
A tooltip on the stats line surfaces the full hub IATAs + pax/cargo/year
breakdown when present.

The alliance logo's `title` tooltip now uses the alliance display name when
known (was bare "Alliance #N"). Existing pax + cargo market-share lines and
the airport weekly-departures line remain unchanged.

**No manifest change in this fix** — see manifest-requests.md follow-up
asking Agent 1 to add `modules/competitor-intel/competitor-store.js` to the
scheduling content_script so future enhancements can use the typed store
API instead of raw key reads.

**Files touched:** `modules/route-assistant/panel.js` only.

**Static check:** `node --check modules/route-assistant/panel.js` PASS.

**Live verification (read-only, no SHARED-NOTES lock needed):** open the
RA panel on JFK, hover any Cmp pill — popover opens, no thrown errors in
DevTools console, stats line populates for competitors whose
competitor-intel record has been captured (or is silent for those without).

---

## [FIXED] Cmp pill hover still froze the AS tab — sync DOM flood (2026-05-03)

**Bug.** The earlier try/catch wrap caught *thrown exceptions* but the user
reported the AS tab still froze on hover — a synchronous main-thread hang,
not an exception. Repro: hovering the **Cmp** pill on a row whose destination
is a busy hub (cached AS airport overview) builds a popover row per carrier
operating from that destination. Busy-hub caches hold 100-300+ enterprises.
Each row creates two `<img>` tags (alliance + banner) pointing at
`/app/logo/<id>/enterprise-s.png?strict=true`, which 404s for any enterprise
without a custom logo. Each 404 fires the `error` listener which mutates the
DOM (removeChild + append placeholder), forcing reflow inside the same
synchronous build loop. Net effect: a few hundred image fetches + DOM
mutations on a single frame, blocking long enough for Chrome's "Page
Unresponsive" prompt.

**Fixes** (`modules/route-assistant/panel.js`):

1. **Cap airport-overview fallback at top 30** by `weeklyDepartures`. The
   fallback is by definition a "what carriers operate from the destination"
   summary; the long tail of single-rotation operators doesn't help
   competition decisions. Truncation count surfaced in a yellow footer line
   linking to the AS airport page for the full list.
2. **Defensive hard cap at 60** on `shares` after source-priority
   resolution. Catches any future scraper that returns a long list. Footer
   message: "Showing top N of M competitors (popover capped for
   performance)."
3. **Lazy-load per-carrier images** — `loading="lazy"` + `decoding="async"`
   on both alliance and banner `<img>` elements. The popover is
   `position:fixed; max-height:70vh; overflowY:auto`, so off-screen rows
   no longer fetch banners until scrolled into view.

**Files touched:** `modules/route-assistant/panel.js` only.

**Static check:** `node --check modules/route-assistant/panel.js` PASS.

**Live verification:** hover Cmp pills on rows pointing at busy hubs (LHR/
DXB/JFK destinations). Popover paints within ~300 ms, page stays
interactive. Truncation footer appears with correct shown/total. Banner
images load progressively as the user scrolls inside the popover. Low-Cmp
rows (1-2 carriers) unchanged.
