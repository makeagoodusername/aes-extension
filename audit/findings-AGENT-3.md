...
## 2026-05-04 — K13 + K15 + K12 conductor learning substrate landed

**Disposition:** [FIXED] / [SHIPPED]

Implemented the next layer of informational organisation features per
`docs/CONDUCTOR-ROADMAP.md`:

### K13 — Outcome-weighted threshold learning (data stream)
- `modules/conductor/baseline-store.js` — EWMA mean + variance per
  `(metric, scope, scopeId)`, scoped by host, with shrinkage prior n0=4 and
  α=2/(N+1) (default N=20). Soft cap 800 entries (LRU prune by `lastAt`).
- `modules/conductor/baseline-driver.js` — subscribes to `conductor:signal`
  and maps maintenance.ratio / maintenance.condition / cash.balance /
  route.profit / ors.rank into baseline updates. 24h chrome.alarms tick
  hooked for forecast-store refresh.
- `modules/conductor/scenario-engine.js` — injects `ctx.baselines` (and
  `ctx.forecasts`) per tick.
- `modules/conductor/outcome-driver.js` — same `ctx` extension on the K10
  evaluator hot path.
- `modules/conductor/scenarios.js` — added `_zThreshold` + `_forecast`
  helpers; ProfitDecay match now requires z ≤ -1.5σ vs learned baseline
  (when ≥4 samples) before firing on a pct trigger, suppressing noise dips.

### K15 — Short-horizon forecasting (data stream)
- `modules/conductor/forecaster.js` — pure-function forecasters
  (`forecastLinear`, `forecastEWMA`, `forecastMarkov`) returning a uniform
  `{model, n, horizon, p10, p50, p90, fit, reason}` envelope. CI heuristic
  uses Z_80 = 1.2816 on residual sd.
- `modules/conductor/forecast-store.js` — cache keyed under
  `aesConductor:forecasts:<server>:<airline>`; refreshes on
  `signal:conductor:baseline:tick`, opportunistically on
  `data:conductor:baseline:updated` (rate-limited to 60s); supports
  `scope:'*'` wildcards (e.g. every observed (hub, dest)).
- `modules/conductor/scenarios.js` — new bundled scenario
  **CashCrunchForecast** (alert tier; horizon 28d; floor default 1M AS$);
  ProfitDecay declares a `forecast:` for route.profit lookup.
- `modules/central-hub/tiles/conductor-tile.js` — `_forecastChip` renders
  `P50 <fmt> / Nw` after the outcome chip when an envelope is reachable
  (either stamped on fire payload or via forecast-store cache).

### K12 — Risk register polish
- `modules/central-hub/tiles/strategy-briefing-tile.js` — dedicated
  "Risk register · N" modal section above the existing Risk card. Renders
  full fire list (severity dot, label, rationale, category, age, openUrl)
  + drift proposals. Hidden when `count==0` and no proposals.

### Cross-territory requests filed
- `audit/manifest-requests.md` — request `forecast-store.js` load line
  (the other three K13/K15 modules are already in the manifest at lines
  298–300, thank you Agent 1).
- `audit/bus-topic-requests.md` — three new topics
  (`data:conductor:baseline:updated`, `signal:conductor:baseline:tick`,
  `data:conductor:forecast:updated`).

### Verification
```
audit/tests/conductor/baseline-driver.test.js   3  pass
audit/tests/conductor/forecaster.test.js        5  pass
audit/tests/conductor/forecast-store.test.js    3  pass
node --check on all 9 modified files            pass
manifest.json JSON-validate                     pass
```

### Invariants honoured
- No new POSTs to AS — all writes are local `chrome.storage.local`.
- All new storage keys under `aesConductor:` namespace.
- Strategy pure-function rule preserved (forecaster has no I/O).
- Bus topics added through registry-request protocol (Agent 6).
- Manifest changes routed through manifest-request protocol (Agent 1).
- New scenarios default to `tier:"alert"`; no auto-apply gates flipped.

### Open items / next session pickup
- The 7 §IV scenarios remaining-deferred per K2 entry (MaintenanceCascade,
  CompetitorEntry/Exit forward variants, FleetIdle utilization-window,
  WaveStress, ServiceProfileDrift, OrsRecoveryWatch) are still blocked on
  K2.1 window-spanning matchers. K13 baselines + K15 forecasts unblock
  CashCrunchForecast and a forward variant of ProfitDecay (both shipped
  here). The remaining five need K2.1 first.
- A visible UI for browsing/resetting baselines and forecasts is deferred
  to a future K6.x tile slice (Conductor "Learning" tab).
- Markov forecaster is wired but unused by any bundled scenario; it's
  available for K20 latent scenario discovery and Strategy Slice 22
  (probabilistic risk fans).
