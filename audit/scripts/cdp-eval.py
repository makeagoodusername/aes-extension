...
def main():
    ...
    try:
        ...
        if world == "main":
            ...
            fallback_ctx = None
            probes = [
                "!!(window.AesAfp && window.AesAfpSubmitBridge && window.AesAfpScheduleStore)",
                "!!(window.AesPerLegAutopricer || window.RouteAssistantPanel || window.AesRoutePriceAutomator)",
                "!!(window.FleetScheduleGridHost || window.FleetScheduleGridColorPicker)",
                "!!(window.CentralHubTileRegistry || window.AESCommandRegistry)",
                "!!(window.AesSettings || window.AesDataBus || window.AesAfp || window.AesAfpFormDriver || window.RouteAssistantSettings || window.RouteAssistantSilentAutoProposers || window.AESSiteSkin || window.CanvasRailController)",
            ]
            for c in iso:
        ...
        eval_timeout_ms = int(sys.argv[sys.argv.index("--timeout-ms") + 1]) if "--timeout-ms" in sys.argv else 8000
        params = {
            "expression": expr,
            "returnByValue": True,
            "awaitPromise": True,
        ...
        deadline = time.time() + max(10, (eval_timeout_ms / 1000) + 2)
        while time.time() < deadline:
            try:
                m = json.loads(ws.recv())
            except Exception:
