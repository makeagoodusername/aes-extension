#!/usr/bin/env python3
"""Restore browser session by submitting the visible login form.
Reads creds from audit/credentials.json and simulates the same user
keystrokes a human would perform on the live login page. Read-only
state inspection only — does not flip any apply.* gates or POST to
any AS pricing/IL/AFP endpoint."""
...
def main(port: int) -> int:
    ...
    wait_id(sid, timeout=10)
    wait_method("Page.loadEventFired", timeout=15)
    time.sleep(3)
    send("Page.navigate", {"url": f"https://{server}/app/redirectAfterLogin"})
    wait_method("Page.loadEventFired", timeout=15)
    time.sleep(3)
