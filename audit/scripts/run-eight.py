...
Reads credentials from audit/credentials.json, then in parallel:
  1. Calls cdp-login-v2.login(port, email, password) — opens /auth/login,
...
Reuses cdp-login-v2.py's login() function as-is; no new dependencies.
...
LOGIN_PY = os.path.join(SCRIPT_DIR, "cdp-login-v2.py")
...
def boot_agent(agent_num: int, port: int, url_tpl: str, creds: dict, login_fn) -> dict:
    ...
    if not is_chrome_url:
        # cdp-login-v2.login(port, email, password) opens /auth/login itself
