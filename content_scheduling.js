...
;(function aesRouteAssistantMain() {
    ...
    const tryMount = () => {
        ...
        if (editorReady || Date.now() - start > 15000) {
            try {
                if (typeof RouteAssistantPanel === "function") {
                    ...
                    if (mounted && typeof mounted.catch === "function") {
                        mounted.catch(e => console.warn("[AES scheduling] route assistant mount failed", e))
                    }
                }
            } catch (e) {
                console.warn("[AES scheduling] route assistant mount failed", e)
            }
