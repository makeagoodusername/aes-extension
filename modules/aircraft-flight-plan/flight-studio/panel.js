...
;(function () {
    ...
    function _runPreview() {
        ...
        // "Create new flight number" button.
        if (typeof window.AesAfpFormDriver.fill === "function") {
            try {
                const p = window.AesAfpFormDriver.fill(formLeg)
                if (p && typeof p.then === "function") {
                    p.then((r) => {
                        if (r && r.ok) {
                            _renderHint("info", "AS form pre-filled — review and click 'Create new flight number' to confirm.")
                        } else if (r && r.missed && r.missed.length) {
                            _renderHint("warn", "AS form partially filled — missed: " + r.missed.join(", "))
                        }
                    }).catch(() => { /* form-driver renders its own error hint */ })
                }
            } catch (_) { /* swallow — dry-run pane still updates */ }
        }
        _renderBody()
