...
;(function () {
        ...
        if (sel.options && typeof sel.options.length === "number") {
            for (let i = 0; i < sel.options.length; i++) {
                if (String(sel.options[i].value) === String(value)) {
                    try { sel.selectedIndex = i } catch (_) {}
                    break
                }
            }
        }
    ...
    function _optionText(opt) {
        return String((opt && (opt.textContent || opt.innerText || opt.text)) || "")
    }

    function _cloneOptionIntoSelect(sel, opt) {
        if (!sel || !opt) return null
        const existing = _findOptByValue(sel, opt.value)
        if (existing) return existing
        const text = _optionText(opt)
        const value = String(opt.value)
        let clone = null
        try {
            const OptionCtor = (typeof window !== "undefined" && window.Option)
                || (typeof Option !== "undefined" && Option)
            clone = OptionCtor ? new OptionCtor(text, value) : null
        } catch (_) { clone = null }
        if (!clone) clone = {value: value, text: text, textContent: text}
        try {
            if (typeof sel.add === "function") sel.add(clone)
            else if (sel.options && typeof sel.options.push === "function") sel.options.push(clone)
            else if (typeof sel.appendChild === "function") sel.appendChild(clone)
            else return null
        } catch (_) {
            return null
        }
        return _findOptByValue(sel, value) || clone
    }

    function _setSelectByIata(sel, iata, donorSel) {
        let opt = _findOptByIata(sel, iata)
        if (!opt && donorSel) {
            const donorOpt = _findOptByIata(donorSel, iata)
            opt = _cloneOptionIntoSelect(sel, donorOpt)
            if (opt) {
                _diag("borrow-select-option", {
                    target: sel && sel.name,
                    donor:  donorSel && donorSel.name,
    ...
    function _selectedIata(sel) {
        if (!sel || !sel.options || sel.selectedIndex == null || sel.selectedIndex < 0) return null
        const opt = sel.options[sel.selectedIndex]
        return _iataText(_optionText(opt))
    }

    function _aircraftLocationIata() {
        const ctxOrigin = (window.AesAfp && window.AesAfp.ctx && window.AesAfp.ctx.currentLocationIata) || null
        return (ctxOrigin && /^[A-Z]{3}$/.test(String(ctxOrigin).toUpperCase()))
            ? String(ctxOrigin).toUpperCase()
            : null
    ...
    function _hubForFormRoute(norm) {
        const origin = String(norm && norm.origin || "").toUpperCase()
        const dest   = String(norm && norm.destination || "").toUpperCase()
        const current = _aircraftLocationIata()
        if (current && (origin === current || dest === current)) return current
        const active = _activeHubIata()
        if (active && (origin === active || dest === active)) return active
        return current || active || null
    }

    function _desiredOrientation(norm) {
        const hub = _hubForFormRoute(norm)
        if (!hub) return null
        const origin = String(norm && norm.origin || "").toUpperCase()
        const dest   = String(norm && norm.destination || "").toUpperCase()
        if (origin === hub && dest && dest !== hub) return {hub, direction: "outbound"}
        if (dest === hub && origin && origin !== hub) return {hub, direction: "inbound"}
        return null
    }

    function _formOrientation(form, hub) {
        if (!form || !hub) return null
        const origin = _selectedIata(form.originSelect)
        const dest   = _selectedIata(form.destSelect)
        if (origin === hub && dest !== hub) return "outbound"
        if (dest === hub && origin !== hub) return "inbound"
        if (origin === hub && dest === hub) return "ambiguous"
        return null
    }

    async function _ensureRouteOrientation(norm) {
        const desired = _desiredOrientation(norm)
        if (!desired) return {ok: true}
        let form = findForm()
        let current = _formOrientation(form, desired.hub)
        if (current === desired.direction) return {ok: true}
        if (current !== "outbound" && current !== "inbound") {
            _diag("route-orient-skip", {
                desired: desired.direction,
                current: current || "unset"
            })
            return {ok: true, skipped: true, reason: "form-route-unset"}
        }
        const btn = form && form.reverseBtn
        if (!btn) return {ok: false, error: "reverse-unavailable", desired: desired.direction, current}
        try { btn.click() }
        catch (e) {
            _diag("route-orient-error", {err: String(e), desired: desired.direction, current})
            return {ok: false, error: "reverse-click-failed", desired: desired.direction, current}
        }
        for (let i = 0; i < 50; i++) {
            await new Promise(r => setTimeout(r, 100))
            form = findForm()
            current = _formOrientation(form, desired.hub)
            if (current === desired.direction) {
                _diag("route-orient-ok", {desired: desired.direction, waitedMs: (i + 1) * 100})
                return {ok: true}
            }
        }
        _diag("route-orient-timeout", {desired: desired.direction, current})
        return {ok: false, error: "reverse-timeout", desired: desired.direction, current}
    }
    ...
    function setOrigin(iata) {
        const f = findForm()
        return _setSelectByIata(f && f.originSelect, iata, f && f.destSelect)
    }
    function setDestination(iata) {
        const f = findForm()
        return _setSelectByIata(f && f.destSelect, iata, f && f.originSelect)
    }

    function _pushMissed(missed, key) {
        if (missed.indexOf(key) < 0) missed.push(key)
    }

    function _verifyRouteFields(norm, set, missed) {
        const f = findForm()
        const actualOrigin = _selectedIata(f && f.originSelect)
        const actualDest   = _selectedIata(f && f.destSelect)
        const expectedOrigin = String(norm.origin || "").toUpperCase()
        const expectedDest   = String(norm.destination || "").toUpperCase()
        if (expectedOrigin && actualOrigin !== expectedOrigin) {
            delete set.origin
            _pushMissed(missed, "origin")
        }
        if (expectedDest && actualDest !== expectedDest) {
            delete set.destination
            _pushMissed(missed, "destination")
        }
        return {origin: actualOrigin, destination: actualDest}
    }
    ...
    async function fill(leg) {
        ...
        if (norm.flightNumberText) {
            ...
            else                                        _pushMissed(missed, "flightNumberText")
        }

        _lastLeg = norm
        const result = {ok: missed.length === 0, set: set, missed: missed}
        if (missed.indexOf("origin") >= 0 || missed.indexOf("destination") >= 0) {
            _renderHint({
                kind: "error",
                message: "Could not match AS form to " + (norm.origin || "?") + " → "
                    + (norm.destination || "?") + ". Current form shows "
                    + (actualRoute.origin || "?") + " → " + (actualRoute.destination || "?") + "."
            })
        } else {
    ...
    function dryRun(leg) {
        ...
        const tryIata  = (sel, donorSel, iata, key) => {
            if (!iata)                     { result.missed.push(key); return }
            const opt = _findOptByIata(sel, iata) || _findOptByIata(donorSel, iata)
        ...
        tryIata(form.originSelect, form.destSelect,   norm.origin,      "origin")
        tryIata(form.destSelect,   form.originSelect, norm.destination, "destination")
        const t = _parseTime(norm.depTime)
        if (t) {
    ...
    function _candidateToFillLeg(candidate) {
        const c = candidate || {}
        if (c.__wave) {
            const leg = {
                origin:      c.__wave.origin || null,
                destination: c.__wave.destination || c.destIata || null,
                depTime:     c.__wave.depTime || undefined
            }
            if (Array.isArray(c.__wave.dayMask)) leg.dayMask = c.__wave.dayMask.slice(0, 7)
            return leg
        }
        return {destination: c.destIata}
    }
    ...
    function _mkBtn(label, cls, onClick) {
        ...
        b.className = cls
        b.textContent = label
        b.style.cssText = "background:#0f1623;color:#cbd5e1;border:1px solid #374151;border-radius:3px;padding:4px 10px;font-size:11px;font-weight:600;cursor:pointer;"
    ...
    function _renderToolbar() {
        ...
        btnRow.append(
            _mkBtn("Fill latest pick",   "aes-afp-btn-fill",    () => {
                ...
                fill(_candidateToFillLeg(_lastCandidate))
            }),
    ...
    function attach() {
        ...
        bus.on("candidate:selected", (payload) => {
            ...
            // clicks Submit).
            const payloadLeg = (p.leg && typeof p.leg === "object") ? p.leg : {}
            const leg = {destination: payloadLeg.destination || p.destinationIata || p.destination || c.destIata}
            const origin = payloadLeg.origin || p.originIata || p.origin || c.originIata || c.origin
            ...
            if (typeof payloadLeg.depTime === "string") leg.depTime = payloadLeg.depTime
            else if (typeof p.depTime === "string") leg.depTime = p.depTime
            if (Array.isArray(payloadLeg.dayMask)) leg.dayMask = payloadLeg.dayMask.slice(0, 7)
            if (Number.isFinite(payloadLeg.pricePct)) leg.pricePct = payloadLeg.pricePct
            if (typeof payloadLeg.service === "string") leg.service = payloadLeg.service
            if (typeof payloadLeg.flightNumberText === "string") leg.flightNumberText = payloadLeg.flightNumberText
            if (p.source === "wave-leg" && c.__wave) {
                ...
                if (Array.isArray(c.__wave.dayMask) && !leg.dayMask) leg.dayMask = c.__wave.dayMask.slice(0, 7)
            }
            fill(leg)
        })
        _loadSettings()
