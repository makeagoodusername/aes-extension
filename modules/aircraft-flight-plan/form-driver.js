...
;(function () {
    ...
    /** Commit a value + repaint the select2 chip. select2 v3's `val` setter
     *  needs `triggerChange=true` as the third arg or the visible chip stays
     *  on the previous label even though `<option selected>` is correct.
     *  v4 has no such signature — use `$.val(value).trigger("change")` instead.
     *  Falls back to native change-event dispatch when jQuery is missing. */
