 ...
 *
 * Inputs:
 *   - Airport multi-select — populated from both AS station dropdowns on
 *     the New Flight Number form, intersected with FlightsFromStore for the
 *     active hub. Reading both selects matters because AS flips the editable
 *     side when the origin/destination direction is reversed. Distance comes
...
;(function () {
    ...
    function _slot() {
        const afp = window.AesAfp || null
        return (afp && typeof afp.slot === "function")
            ? afp.slot(SLOT_NAME) : null
    }
    ...
    function _selectOptions(sel) {
        ...
        try { return Array.from(sel.options) } catch (_) { return [] }
    }
    ...
    function _iataFromOption(opt) {
        const text = _optionLabel(opt)
        if (!text) return null
    ...
    function _selectedIata(sel) {
        ...
        return _iataFromOption(_selectOptions(sel)[sel.selectedIndex])
    }

    function _readSelectAirports(sel, hub) {
        const out = new Map()
        const hubIata = String(hub || "").toUpperCase()
        for (const opt of _selectOptions(sel)) {
            const iata = _iataFromOption(opt)
            if (!iata || iata === hubIata) continue
            if (out.has(iata)) continue
            out.set(iata, {
                iata,
                label: _optionLabel(opt) || iata,
                asValue: opt && opt.value != null ? opt.value : ""
            })
        }
        return out
    }

    function _mergeAsCandidate(map, side, candidate) {
        if (!candidate || !candidate.iata) return
        let entry = map.get(candidate.iata)
        if (!entry) {
            entry = {
                iata: candidate.iata,
                label: candidate.label,
                asValue: candidate.asValue,
                inOriginSelect: false,
                inDestSelect: false,
                originAsValue: null,
                destAsValue: null,
                directions: {inbound: false, outbound: false}
            }
            map.set(candidate.iata, entry)
        }
        if (!entry.label && candidate.label) entry.label = candidate.label
        if (!entry.asValue && candidate.asValue) entry.asValue = candidate.asValue
        if (side === "origin") {
            entry.inOriginSelect = true
            entry.originAsValue = candidate.asValue
            entry.directions.inbound = true
        } else if (side === "dest") {
            entry.inDestSelect = true
            entry.destAsValue = candidate.asValue
            entry.directions.outbound = true
        }
    }

    function _directionLabel(a) {
        if (!a || !a.inAsDropdown) return ""
        if (a.directions && a.directions.inbound && a.directions.outbound) return "in/out"
        if (a.directions && a.directions.outbound) return "out"
        if (a.directions && a.directions.inbound) return "in"
        return "permit"
    }
     ...
     * order:
     *   1. AS origin + destination dropdown options on the New Flight Number
     *      form (the gate that decides what's actually fillable). We read
     *      both because the editable select moves when AS reverses the form.
    ...
    async function _gatherCandidateAirports(ctx, hub) {
        ...
        const seen = new Set()
        const afp = window.AesAfp || null
        const formTabs = afp && typeof afp.getNewFlightForm === "function"
            ? afp.getNewFlightForm()
            : null
        const originSelect = formTabs && formTabs.originSelect
        const destSelect = formTabs && formTabs.destSelect
        const originIata = _selectedIata(originSelect)
        const destIata = _selectedIata(destSelect)
        const dropdown = new Map()
        for (const dd of _readSelectAirports(originSelect, hub).values()) {
            _mergeAsCandidate(dropdown, "origin", dd)
        }
        for (const dd of _readSelectAirports(destSelect, hub).values()) {
            _mergeAsCandidate(dropdown, "dest", dd)
        }
        let ff = null
        const ffStore = window.FlightsFromStore || null
        if (hub && ffStore && typeof ffStore.loadAirport === "function") {
            try { ff = await ffStore.loadAirport(hub) } catch (_) { ff = null }
        }
        ...
        for (const iata of allIatas) {
            ...
            out.push({
                ...
                originAsValue:  dd ? dd.originAsValue : null,
                destAsValue:    dd ? dd.destAsValue : null,
                inAsDropdown:   !!dd,
                inOriginSelect: !!(dd && dd.inOriginSelect),
                inDestSelect:   !!(dd && dd.inDestSelect),
                directions:      dd ? dd.directions : {inbound: false, outbound: false},
                currentForm:     {origin: originIata, destination: destIata},
                inFlightsFrom:  !!ffr,
    ...
    async function render(ctx) {
        ...
        for (const a of candidates) {
            ...
            const tagsParts = []
            const permitLabel = _directionLabel(a)
            if (!a.inAsDropdown)  tagsParts.push("<span style='color:" + COLOR_HOT + "'>no permit</span>")
            else if (permitLabel) tagsParts.push(permitLabel)
    ...
    function _wireBus() {
        const afp = window.AesAfp || null
        if (!afp || !afp.bus) return
        let pending = false
        const tick = () => {
            if (pending) return
            pending = true
            setTimeout(() => { pending = false; render(afp.ctx) }, 100)
        }
        afp.bus.on("ctx:ready", tick)
        afp.bus.on("spec:resolved", tick)
        afp.bus.on("flightsfrom:ready", tick)
    ...
    window.AesAfpMockScheduleStudio = {
        ...
        SLOT_NAME,
        _internal: {
            gatherCandidateAirports: _gatherCandidateAirports,
            readSelectAirports: _readSelectAirports,
            iataFromOption: _iataFromOption,
