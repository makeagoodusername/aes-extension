...
class CanvasShell {
    ...
    _buildHeader(T) {
        ...
        // Row-mode toggle (Aircraft × Day | Day Overlay) — drives both the
        // Timeline view's row dispatch and the Wave Spine's per-cell
        // mini-Gantt vs day-pill rendering.
        const rowToggle = this._buildSegmented(T,
            [
                {id: "aircraftByDay", label: "Aircraft × Day"},
                {id: "dayOverlay",    label: "Day Overlay"}
            ],
            this._state.rowMode || "aircraftByDay",
            (id) => this._setRowMode(id))
        wrap.append(rowToggle)

        wrap.append(this._sep(T))

        // Color-mode toggle.
        const colorToggle = this._buildSegmented(T,
            [
                {id: "route",    label: "Route"},
                {id: "aircraft", label: "Aircraft"},
                {id: "day",      label: "Day"}
            ],
            this._state.colorMode || "route",
            (id) => this._setColorMode(id))
        wrap.append(colorToggle)
    ...
    async _setRowMode(mode) {
        const next = (mode === "dayOverlay") ? "dayOverlay" : "aircraftByDay"
        if (next === (this._state.rowMode || "aircraftByDay")) return
        this._state = Object.assign({}, this._state, {rowMode: next})
        try { await window.AesCanvasStateStore.save({rowMode: next}) } catch (_) {}
        this._renderHeader()
        // Push to whichever view is currently mounted.
        if (this.updateTimelineView) {
            try { this.updateTimelineView({rowMode: next}) } catch (_) {}
        }
        if (this._spineRenderer && typeof this._spineRenderer.setRowMode === "function") {
            this._spineRenderer.setRowMode(next)
        }
    }

    async _setColorMode(mode) {
        const next = (mode === "aircraft" || mode === "day") ? mode : "route"
        if (next === (this._state.colorMode || "route")) return
        this._state = Object.assign({}, this._state, {colorMode: next})
        try { await window.AesCanvasStateStore.save({colorMode: next}) } catch (_) {}
        this._renderHeader()
        if (this.updateTimelineView) {
            try { this.updateTimelineView({colorMode: next}) } catch (_) {}
        }
        if (this._spineRenderer && typeof this._spineRenderer.setColorMode === "function") {
            this._spineRenderer.setColorMode(next)
        }
    }
    ...
    _renderCurrentView() {
        ...
        this._spineRenderer = new CanvasWaveSpineRenderer({
            ...
            preset:    this._currentPreset(),
            focusedAircraftId: this._state.focusedAircraftId,
            rowMode:   this._state.rowMode   || "aircraftByDay",
            colorMode: this._state.colorMode || "route"
        })
        this._spineRenderer.render()
