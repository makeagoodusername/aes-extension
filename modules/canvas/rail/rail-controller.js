...
class CanvasRailController {
    ...
    constructor(deps) {
        ...
        this.advisorEngine = (d.advisorEngine !== undefined) ? d.advisorEngine
            : (typeof window !== "undefined" && window.CanvasAdvisorEngine
                ? new window.CanvasAdvisorEngine({
                    getInputs: this.getInputs,
                    dispatch: {
                        stage:   (e) => this.stageEdits(e),
                        unstage: (pred) => this._unstageMatching(pred)
                    }
                }) : null)
    ...
    _renderAdvisorPane() {
        ...
        const continuousPill = this._buildContinuousTickPill(T)
        proposerRow.append(proposerBtn, proposerStatus, continuousPill)
        this._advisorPaneEl.append(proposerRow)
        this._continuousPillEl = continuousPill
        // Refresh the tick state whenever the Advisor pane re-renders so the
        // pill reflects the latest setting + gating status.
        this._startContinuousTickIfEnabled().catch(() => {})
    ...
    /**
     * Remove staged edits matching a predicate descriptor. Used by advisor
     * remediation actions ("Re-route", "Discard duplicate") to surgically
     * unstage one edit without touching the rest of the batch.
     *
     * pred shape: {matchKind?, aircraftId?, destIata?, waveId?}
     * Returns the count of edits removed.
     */
    _unstageMatching(pred) {
        if (!pred || typeof pred !== "object") return 0
        const want = (k, v) => v == null ? null : String(v)
        const wantKind = want("matchKind", pred.matchKind)
        const wantAid  = want("aircraftId", pred.aircraftId)
        const wantDest = pred.destIata ? String(pred.destIata).toUpperCase() : null
        const wantWave = want("waveId", pred.waveId)
        const removed = []
        const kept = []
        for (const e of this._stagedEdits) {
            const p = (e && e.payload && typeof e.payload === "object") ? e.payload : e
            const eKind = e && e.kind || (p && p.kind) || ""
            const eAid  = p && p.aircraftId != null ? String(p.aircraftId) : ""
            const eDest = String((p && (p.destIata || p.destination)) || "").toUpperCase()
            const eWave = p && p.waveId != null ? String(p.waveId) : ""
            if (wantKind && eKind !== wantKind) { kept.push(e); continue }
            if (wantAid  && eAid  !== wantAid)  { kept.push(e); continue }
            if (wantDest && eDest !== wantDest) { kept.push(e); continue }
            if (wantWave && eWave !== wantWave) { kept.push(e); continue }
            removed.push(e)
        }
        if (!removed.length) return 0
        this._stagedEdits = kept
        if (this._railShell) this._railShell.setStagedCount(this._stagedEdits.length)
        // Clear builder-preview visual markers for the unstaged cells.
        if (typeof document !== "undefined") {
            for (const e of removed) {
                const p = (e && e.payload && typeof e.payload === "object") ? e.payload : e
                const aid = p && p.aircraftId != null ? String(p.aircraftId).replace(/"/g, "\\\"") : ""
                const wid = p && p.waveId != null ? String(p.waveId).replace(/"/g, "\\\"") : ""
                if (!aid || !wid) continue
                const cell = document.querySelector('[data-canvas-aircraft-id="' + aid + '"][data-canvas-wave-id="' + wid + '"]')
                if (!cell) continue
                cell.dataset.canvasBuilderPreview = ""
                cell.style.outline = ""
                cell.style.outlineOffset = ""
                cell.style.background = ""
                const tag = cell.querySelector("[data-canvas-builder-preview-label]")
                if (tag && tag.parentElement) tag.parentElement.removeChild(tag)
            }
        }
        if (typeof window !== "undefined" && window.CentralHubBus) {
            const batchId = "u-" + Date.now().toString(36)
            window.CentralHubBus.emit(window.AesCanvasEvents.EDIT_DISCARDED,
                {batchId, count: removed.length, source: "advisor-action"})
        }
    ...
    /**
     * Start the canvas-scoped continuous proposer tick when the user has
     * opted in via `pricing.canvasContinuousProposers.enabled` AND the
     * panel.js silent-auto loop is OFF (the two surfaces would otherwise
     * double-fire on the same routes). The tick lives only while the rail
     * controller is mounted and the user is in Advisor mode.
     */
    async _startContinuousTickIfEnabled() {
        this._stopContinuousTick()
        if (this._currentMode() !== window.CanvasRailShell.MODE_ADVISOR) return
        if (typeof window === "undefined" || !window.RouteAssistantSettings) return
        let settings = null
        try { settings = await window.RouteAssistantSettings.load() }
        catch (_) { return }
        const p = (settings && settings.pricing) || {}
        const cfg = p.canvasContinuousProposers || {}
        if (cfg.enabled !== true) {
            this._continuousTickReason = "off"
            this._refreshContinuousTickPill()
            return
        }
        if (p.silentAutoEnabled === true) {
            this._continuousTickReason = "blocked"
            this._refreshContinuousTickPill()
            return
        }
        const intervalMin = Math.max(2, Number(cfg.intervalMin) || 5)
        const intervalMs = intervalMin * 60 * 1000
        this._continuousTickReason = "running"
        this._continuousTickIntervalMin = intervalMin
        this._continuousTickHandle = setInterval(() => {
            // Re-check mode at fire time; the user may have flipped to
            // Builder while we were waiting.
            if (this._currentMode() !== window.CanvasRailShell.MODE_ADVISOR) {
                this._stopContinuousTick()
                return
            }
            this._runProposerSurfaceTick().catch(err =>
                console.warn("[AES Canvas] continuous proposer tick failed", err))
        }, intervalMs)
        this._installContinuousTickSettingsWatcher()
        this._refreshContinuousTickPill()
    }

    _stopContinuousTick() {
        if (this._continuousTickHandle) {
            try { clearInterval(this._continuousTickHandle) } catch (_) {}
            this._continuousTickHandle = null
        }
    }

    _installContinuousTickSettingsWatcher() {
        if (this._continuousTickWatcherInstalled) return
        if (typeof chrome === "undefined" || !chrome.storage || !chrome.storage.onChanged) return
        const listener = (_changes, area) => {
            if (area !== "local") return
            // Cheap path: any local-area change triggers a re-evaluate. The
            // settings store is a single key so the cost is one async load.
            this._startContinuousTickIfEnabled().catch(() => {})
        }
        chrome.storage.onChanged.addListener(listener)
        this._continuousTickWatcherInstalled = true
        this._busOff.push(() => {
            try { chrome.storage.onChanged.removeListener(listener) } catch (_) {}
            this._continuousTickWatcherInstalled = false
        })
    }

    _buildContinuousTickPill(T) {
        const pill = document.createElement("button")
        pill.type = "button"
        pill.title = "Open Settings to toggle the continuous proposer tick"
        pill.style.cssText = [
            "padding:2px 8px",
            "font-size:9px",
            "letter-spacing:0.06em",
        ...
        pill.textContent = "Continuous: …"
        pill.addEventListener("click", () => {
            if (typeof window !== "undefined" && window.CentralHubBus) {
                window.CentralHubBus.emit("open-tile", {tileId: "settings"})
            }
        })
        return pill
    }

    _refreshContinuousTickPill() {
        const el = this._continuousPillEl
        if (!el) return
        const T = (typeof window !== "undefined" && window.AESTokens) || null
        const reason = this._continuousTickReason || "off"
        if (reason === "running") {
            el.textContent = "Continuous: every " + this._continuousTickIntervalMin + "m"
            el.style.background = T ? T.color.bone3 : "#E0DAC8"
            el.style.borderColor = T ? T.color.cobalt : "#3656A8"
            el.style.color = T ? T.color.cobalt : "#3656A8"
        } else if (reason === "blocked") {
            el.textContent = "Continuous: blocked (silent-auto on)"
            el.style.background = T ? T.color.bone : "#F4F1EA"
            el.style.borderColor = T ? T.color.amber : "#B8861F"
            el.style.color = T ? T.color.amber : "#B8861F"
        } else {
            el.textContent = "Continuous: off"
            el.style.background = T ? T.color.bone : "#F4F1EA"
            el.style.borderColor = T ? T.color.paperRule : "#C9C0B0"
            el.style.color = T ? T.color.slate : "#7A6F66"
        }
    }
