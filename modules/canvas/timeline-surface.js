...
class CanvasTimelineSurface {
    ...
    constructor(opts) {
        ...
        this.rowMode     = o.rowMode   || "aircraftByDay"
        this.colorMode   = o.colorMode || "route"
    ...
    mount(rootEl) {
        ...
        // Hydrate overrides + watch for cross-tab edits.
        this._wireColorOverrides()
    ...
    _wireColorOverrides() {
        ...
        this._overridesUnwatch = window.AesScheduleColorOverrides.watch((next) => {
    ...
    update(opts) {
        ...
        if (o.rowMode   !== undefined) this.rowMode   = o.rowMode   || "aircraftByDay"
        if (o.colorMode !== undefined) this.colorMode = o.colorMode || "route"
        this._renderAll()
    }
    ...
    _renderAll() {
        ...
        if (typeof FleetScheduleGridColoring !== "undefined") {
            try { this.coloring = FleetScheduleGridColoring.assign(this.schedules, {overrides: this._overrides}) }
        ...
        this._renderer._rowMode = this.rowMode
        this._renderer._colorMode = this.colorMode
        this._renderer._overrides = this._overrides
    ...
    _onFlightMouseDown(e) {
        ...
        if (!blockEl) return
        if (e.target && e.target.closest && e.target.closest(".aes-fsg-wave-band")) return
        // In aircraftByDay mode, the lane carries data-aircraft-id; in
        // dayOverlay mode, only the block does. Resolve via either path.
        const lane = blockEl.closest("[data-day-idx]")
        if (!lane) return
    ...
    _openInspectorForBlock(blockEl, dragResult) {
        ...
        const lane = blockEl.closest("[data-day-idx]")
        if (!lane) return
        // Block carries data-aircraft-id in both modes (renderer always
        // stamps it). Lane carries it only in aircraftByDay mode.
        const aircraftId = blockEl.dataset.aircraftId || lane.dataset.aircraftId
