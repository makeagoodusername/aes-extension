...
class CanvasWaveSpineRenderer {
    ...
    constructor(deps) {
        ...
        this.rowMode   = d.rowMode   || "aircraftByDay"
        this.colorMode = d.colorMode || "route"
        this.overrides = d.overrides || null
        this._overridesUnwatch = null
        if (typeof window !== "undefined" && window.AesScheduleColorOverrides) {
            window.AesScheduleColorOverrides.load().then((ov) => {
                this.overrides = ov
                this.render()
            }).catch(() => {})
            this._overridesUnwatch = window.AesScheduleColorOverrides.watch((ov) => {
                this.overrides = ov
                this.render()
            })
        }
    ...
    dispose() {
        if (this._overridesUnwatch) {
            try { this._overridesUnwatch() } catch (_) {}
            this._overridesUnwatch = null
        }
    }
    ...
    update(deps) {
        ...
        if (deps.rowMode   !== undefined) this.rowMode = deps.rowMode || "aircraftByDay"
        if (deps.colorMode !== undefined) this.colorMode = deps.colorMode || "route"
    }

    /**
     * Resolve a per-cell, per-day fill color under the active colorMode.
     * Falls back to the cell's route-derived `summary.color` when no
     * override-aware path is available.
     */
    _resolveCellColor(row, wave, summary, dayIdx) {
        const C = (typeof FleetScheduleGridColoring !== "undefined") ? FleetScheduleGridColoring : null
        if (this.colorMode === "aircraft" && C) {
            return C.colorOfAircraft(row.aircraftId, this.overrides).fill
        }
        if (this.colorMode === "day" && C) {
            return C.colorOfDay(dayIdx, this.overrides).fill
        }
        // Route mode — honor byRoute override on summary's top route, else
        // the legacy summary color.
        const routeKey = summary && summary.topRouteKey
        if (routeKey && this.overrides && this.overrides.byRoute && this.overrides.byRoute[routeKey]) {
            return this.overrides.byRoute[routeKey]
        }
        return (summary && summary.color) || "#3656A8"
    }
    ...
    _buildScheduleCell(T, row, wave) {
        ...
        if (isEmpty) {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                const fill = filled ? this._resolveCellColor(row, wave, summary, i) : "transparent"
                pill.style.cssText = [
                    "width:14px",
                    "height:10px",
                    "display:inline-flex",
                    ...
                    "background:" + fill,
                    "border:1px solid " + (filled ? fill : (T ? T.color.paperRule : "#C9C0B0")),
                    "border-radius:1px"
                ].join(";")
            ...
            el.append(head, days)
            if (this.rowMode === "dayOverlay") {
                const strip = this._buildLayeredDayStrip(T, row, wave, summary)
    ...
    /**
     * Mini 24h strip overlaying each day's flight legs in this wave window.
     * One thin row per day (Mon..Sun); each row uses opacity + multiply
     * blend so simultaneous flights across days darken the strip — same
     * visual idiom as the timeline's Day Overlay mode.
     */
    _buildLayeredDayStrip(T, row, wave, summary) {
        const sched = this.schedules.get(String(row.aircraftId))
        if (!sched || !Array.isArray(sched.days)) return null
        const wrap = document.createElement("div")
        wrap.style.cssText = "position:relative;height:32px;margin-top:5px;"
            + "background:" + (T ? T.color.bone : "#F4F1EA") + ";"
            + "border:1px solid " + (T ? T.color.paperRule : "#C9C0B0") + ";"
            + "overflow:hidden;"
        wrap.title = "All days overlaid · " + (summary.flightCount || 0) + " flights"
        // 6h gridlines.
        for (let h = 6; h < 24; h += 6) {
            const tick = document.createElement("div")
            tick.style.cssText = "position:absolute;left:" + ((h / 24) * 100)
                + "%;top:0;bottom:0;width:1px;background:" + (T ? T.color.bone3 : "#E0DAC8") + ";"
        ...
        for (let dayIdx = 0; dayIdx < 7; dayIdx++) {
            ...
            for (const b of day.blocks) {
                if (!b || b.startMin == null || b.durationMin == null || b.durationMin <= 0) continue
                if (b.kind !== "flight" && b.kind !== "maintenance") continue
                const left = (b.startMin / 1440) * 100
                const width = Math.max(0.4, (b.durationMin / 1440) * 100)
                const bar = document.createElement("div")
                let fill
                if (b.kind === "maintenance") {
                    fill = (this.overrides && this.overrides.maintenance)
                        || (window.AesScheduleColorOverrides && window.AesScheduleColorOverrides.defaultMaintenance())
                        || "#1A1612"
                } else {
                    fill = this._resolveCellColor(row, wave, summary, dayIdx)
                }
                bar.style.cssText = "position:absolute;top:3px;bottom:3px;"
                    + "left:" + left + "%;width:" + width + "%;"
                    + "background:" + fill + ";opacity:0.5;mix-blend-mode:multiply;"
                    + "border-radius:1px;"
