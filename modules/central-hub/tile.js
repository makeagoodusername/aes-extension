        ...
        if (this.badgeEl) {
            ...
            if (freshness && freshness.stale) {
                if (window.AESFreshnessPill && typeof window.AESFreshnessPill.mount === "function") {
                    ...
                    pillHost.className = "aes-central-hub-tile__freshness"
                    pillHost.style.cssText = "display:inline-flex;margin-left:" +
                        ((window.AESTokens && window.AESTokens.sp) ? window.AESTokens.sp[1] : "4px") +
                        ";vertical-align:middle"
                    this.badgeEl.appendChild(pillHost)
                    window.AESFreshnessPill.mount(pillHost, {
                        ageMs: freshness.ageMs,
                        ttlMs: freshness.ttlMs,
                        stale: true,
                        dense: true,
                        ariaLabel: "Cached value is stale" +
                    ...
                    dot.setAttribute("aria-label", "Cached value is stale" +
                        (freshness.ageMs ? " (" + Math.round(freshness.ageMs / 1000) + "s old)" : ""))
                    ...
                    dot.style.cssText = [
                        ...
                        "height:6px",
                        "border-radius:50%",
                        "background:" + (T && T.color ? T.color.slate : "currentColor"),
