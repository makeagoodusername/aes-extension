...
;(function () {
    ...
    class CentralHubDriftTile extends window.CentralHubTile {
        ...
        async _compute() {
            ...
            if (!host) return {drifted: [], proposals: [], reason: "no host"}
            ...
            return {drifted, proposals: recent, reason: null}
        }
        ...
        async renderBody(_ctx, hostEl) {
            ...
            foot.textContent = "Apply gate is dry-run by default. Live application writes to aesConductor:thresholds and rationales reference the overlay."
        ...
        _renderProposalRow(p, T) {
            ...
            acceptBtn.textContent = p.accepted ? "Applied" : "Apply (dry-run)"
            acceptBtn.disabled = !!p.accepted
            acceptBtn.style.cssText = "background:transparent;border:1px solid " + (T && T.color.slate || "rgba(148,163,184,0.35)")
                + ";color:" + (T && T.color.text || "#e2e8f0")
            ...
            acceptBtn.addEventListener("click", async () => {
                ...
                if (!host || !ts) return
                // Default to dry-run; user flips the per-account gate via console
                // or future settings card. Either way, apply() emits the
                // bus event so the tile + journal can show the preview.
