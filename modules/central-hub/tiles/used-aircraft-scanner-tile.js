...
class CentralHubUasTile extends window.CentralHubTile {
    ...
    /**
     * HubFeed slice — the canonical newest-scan + best-deal projection.
     * The base class auto-subscribes; refreshes fire on view recompute and
     * the freshness dot lights once the slice's TTL elapses.
     */
    feedSlices() { return ["hub:scanner:deals"] }
    ...
    async loadStatus() {
        ...
        if (session && session.finishedAt) {
            ...
            const diffTail = (latest && latest.summary && latest.diffCounts && !latest.diffCounts.firstScan)
                ? " · " + latest.summary
                : ""
            // hub:scanner:deals — when the cached view has a bestDeal,
            // surface it inline so the badge tells the user *what* is
            // currently the best pick instead of just the scan count.
            const dealsView = (window.HubFeed && window.HubFeed.read)
                ? window.HubFeed.read("hub:scanner:deals") : null
            const best = dealsView && dealsView.bestDeal
            const dealTail = best && best.type
                ? " · best: " + best.type
                : ""
            return {
