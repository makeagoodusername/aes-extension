...
;(function () {
    ...
    // IATA → enterpriseId backfill — see _stampIataMapping() for the writer
    // and _resolveIataMappings() for the reader. Used to upgrade IATA-only
    // carriers (markets-derived rows) into real enterprise ids once we've
    // observed the airline's full record at least once.
    const IATA_MAP_PREFIX   = "competitorIntel:iataToEnterpriseId:"
    ...
    async function loadServerData(server) {
        ...
        const iataBackfill = await _loadIataBackfill(server, enterprises, edges)
        const ourSchedule = _buildOurScheduleIndex(server, all)
        const demand = _buildDemandIndex(all)
        return {
            server, enterprises, snapshots, edges, orsRoutes, ourHubs,
            iataBackfill, ourSchedule, demand,
            scannedAt: Date.now()
        }
    }

    /**
     * Pure projection of `aircraftFlightPlan:schedule:<server>:*` into a
     * per-airport leg-count index used by the explore-map "Our network"
     * overlay. Counts each leg once at origin and once at destination, so
     * a 14-leg-per-week aircraft flying LHR↔CDG round-trips contributes 14
     * to LHR and 14 to CDG. Empty when no AFP schedules are stored on this
     * server.
     *
     *   all: {<key>: <value>} from chrome.storage.local.get(null)
     * Returns Map<iata, {departures, arrivals, totalLegs, aircraftIds:Set}>
     */
    function _buildOurScheduleIndex(server, all) {
        const out = new Map()
        if (!server || !all) return out
        const PREFIX = "aircraftFlightPlan:schedule:" + String(server) + ":"
        const bump = (iata, kind, aircraftId) => {
            const code = String(iata || "").toUpperCase()
            if (!/^[A-Z]{3}$/.test(code)) return
            const slot = out.get(code) || {departures: 0, arrivals: 0, totalLegs: 0, aircraftIds: new Set()}
            if (kind === "dep") slot.departures += 1
            else if (kind === "arr") slot.arrivals += 1
            slot.totalLegs += 1
            if (aircraftId != null) slot.aircraftIds.add(String(aircraftId))
            out.set(code, slot)
        }
        for (const k in all) {
            ...
            for (const leg of rec.legs) {
                if (!leg) continue
                if (leg.origin) bump(leg.origin, "dep", aircraftId)
                if (leg.destination) bump(leg.destination, "arr", aircraftId)
            }
        }
        return out
    }
    ...
    /**
     * Pure projection of `routeAssistant:demand:<IATA>` into a per-airport
     * demand index used by the explore-map "Demand" overlay. Read-only on
     * the same chrome.storage.local snapshot. Empty entries (no scrape yet)
     * are skipped so the legend's "no data" tooltip doesn't lie.
     *
     *   all: {<key>: <value>} from chrome.storage.local.get(null)
     * Returns Map<iata, {paxScore, cargoScore, sizeScore, scrapedAt}>
     */
    function _buildDemandIndex(all) {
        const out = new Map()
        if (!all) return out
        const PREFIX = "routeAssistant:demand:"
        for (const k in all) {
            if (k.indexOf(PREFIX) !== 0) continue
            const rec = all[k]
            if (!rec || typeof rec !== "object") continue
            const iata = String(rec.iata || k.slice(PREFIX.length) || "").toUpperCase()
            if (!/^[A-Z]{3}$/.test(iata)) continue
            const pax = Number(rec.paxScore)
            const cargo = Number(rec.cargoScore)
            const size = Number(rec.sizeScore)
            // Skip records whose every scoring field is missing — keeps the
            // legend honest (no zero-weight bubbles claiming "demand").
            if (!isFinite(pax) && !isFinite(cargo) && !isFinite(size)) continue
            out.set(iata, {
                paxScore:   isFinite(pax) ? pax : null,
                cargoScore: isFinite(cargo) ? cargo : null,
                sizeScore:  isFinite(size) ? size : null,
    ...
    /**
     * Bulk-seed the iata→enterpriseId backfill from this server's enterprises
     * cache, then return the resulting Map for downstream views. Keeps the
     * table fresh without forcing every consumer to write back individually.
     * Failures are non-fatal — the views fall back to the synthetic "iata:XX"
     * id path when the table is unavailable.
     */
    async function _loadIataBackfill(server, enterprises, edges) {
        const ns = window.AesCompetitorIataBackfill
        if (!ns) return new Map()
        try {
            const fromEnt = ns.projectFromEnterprises(enterprises)
            const fromEdges = typeof ns.projectFromEdges === "function"
                ? ns.projectFromEdges(edges) : []
            // Enterprise records have richer name fields and win on conflict —
            // they go second so their entries overwrite any edge-derived stub.
            const triples = fromEdges.concat(fromEnt)
            if (triples.length) await ns.bulkSeed(server, triples)
            return await ns.loadAll(server)
        } catch (e) {
            console.warn("[AES competitor-intel] iata-backfill load failed", e)
            return new Map()
        }
    }

    /**
     * Single-shot writer used when a downstream surface (e.g. a Profile click
     * on a markets-derived carrier row that finally got an enterpriseId) wants
     * to teach the table a new pairing without rebuilding from the full
     * enterprises Map. No-op when AesCompetitorIataBackfill isn't loaded yet.
     */
    async function _stampIataMapping(server, iata, enterpriseId, name) {
        const ns = window.AesCompetitorIataBackfill
        if (!ns) return false
        try {
            return await ns.record(server, iata, enterpriseId, name)
        } catch (e) {
            console.warn("[AES competitor-intel] iata-backfill stamp failed", e)
            return false
        }
    }

    /**
     * Batch read used when a caller has a list of IATAs (without an attached
     * data envelope) and needs to resolve as many as possible at once.
     * Returns a Map<IATA, {enterpriseId, name, lastSeenAt}> keyed by
     * upper-case 2/3-letter codes; missing codes are absent from the map.
     */
    async function _resolveIataMappings(server, iatas) {
        const out = new Map()
        const ns = window.AesCompetitorIataBackfill
        if (!ns || !iatas || !iatas[Symbol.iterator]) return out
        try {
            const all = await ns.loadAll(server)
            for (const raw of iatas) {
                const code = String(raw || "").toUpperCase()
                if (!code) continue
                const hit = all.get(code)
                if (hit) out.set(code, hit)
            }
        } catch (e) {
            console.warn("[AES competitor-intel] iata-backfill resolve failed", e)
        }
        return out
    }
    ...
    window.AesCompetitorIntelHost = {
        ...
        loadServerData,
        currentServer,
        stampIataMapping: _stampIataMapping,
        resolveIataMappings: _resolveIataMappings,
        _buildOurScheduleIndex,
        _buildDemandIndex
    }
})()
