...
;(function () {
    ...
    function _onPickAirport(iata, data, paneHost, opts) {
        ...
        for (const c of list) {
            ...
            const hasRealId = c.enterpriseId && !String(c.enterpriseId).startsWith("iata:")
            profileBtn.textContent = hasRealId ? "Profile →" : "Search ↗"
            profileBtn.title = hasRealId
                ? "Open this airline's enterprise page on AS in a new tab. AES will inject the competitor profile panel inline."
                : "No enterprise id known yet — opens AS info-search by IATA. Click a flight on the carrier's route to seed the enterprise id."
            ...
            profileBtn.addEventListener("click", () => {
                if (hasRealId) {
                    window.open("/app/info/enterprises/" + encodeURIComponent(c.enterpriseId), "_blank")
                } else if (c.iata) {
    ...
    function render(host, data, opts) {
        ...
        mapTitle.innerHTML = "<span><b style='color:#7dd3fc;'>EXPLORE</b> · "
            ...
            + "</span>"
        // Layer chip group — toggles bubble color/size between carrier
        // count (default) and ORS connection density. ORS chip disabled
        // when no ORS records have been scraped.
        const chips = document.createElement("span")
        chips.style.cssText = "display:inline-flex;gap:4px;align-items:center;"
        const mkChip = (id, label, hint, enabled) => {
            const b = document.createElement("button")
            b.type = "button"
            b.dataset.layer = id
            b.textContent = label
            b.title = hint + (enabled ? "" : " (no data — scrape ORS for at least one route to enable)")
            b.disabled = !enabled
        ...
        const carriersChip = mkChip("carriers", "Carriers", "Bubble size = carriers cached at airport, color = our hub / ≥5 / default.", true)
        const orsChip = mkChip("ors", "ORS connections", "Bubble size = ORS total-connections summed across routes, color = traffic intensity.", orsIdx.size > 0)
        chips.append(carriersChip, orsChip)
        ...
        function paintBubbles() {
            for (const b of bubbles) {
                const r = b.rec
                const orsSlot = orsIdx.get(r.iata) || null
                const layer = activeLayer
                const sizeWeight = layer === "ors"
                    ? (orsSlot ? orsSlot.connections : 0)
                    : r.carriers.size
                const sizeMax = layer === "ors" ? maxOrsConn : maxCarriers
                const radius = _radius(sizeWeight, sizeMax)
                let fill, stroke
                if (r.isOurs) {
                    fill = "#10b981"; stroke = "#a7f3d0"
                } else if (layer === "ors" && orsSlot) {
                    // Gradient from cyan (low) → magenta (high)
                    const t = Math.max(0, Math.min(1, orsSlot.connections / Math.max(1, maxOrsConn)))
                    if (t > 0.66)      { fill = "#ec4899"; stroke = "#831843" }
                    else if (t > 0.33) { fill = "#a855f7"; stroke = "#581c87" }
                    else               { fill = "#38bdf8"; stroke = "#0c4a6e" }
                } else {
                    fill = r.carriers.size >= 5 ? "#f59e0b" : "#38bdf8"
                    stroke = "#0c4a6e"
                }
                ...
                // Tooltip refresh per layer
                const tipEl = b.circle.querySelector("title")
                if (tipEl) {
                    tipEl.textContent = r.iata + (r.name ? " · " + r.name : "")
                        + (layer === "ors"
                            ? " · " + (orsSlot ? orsSlot.connections + " ORS conns / " + orsSlot.routeCount + " routes" : "no ORS")
                            : " · " + r.carriers.size + " carrier(s)"
        ...
        function applyChipState() {
            for (const c of [carriersChip, orsChip]) {
        ...
        carriersChip.addEventListener("click", () => {
            if (activeLayer === "carriers") return
            activeLayer = "carriers"
            applyChipState()
            paintBubbles()
        })
        orsChip.addEventListener("click", () => {
            if (orsChip.disabled || activeLayer === "ors") return
            activeLayer = "ors"
            applyChipState()
            paintBubbles()
        })
