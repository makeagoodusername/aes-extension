 ...
 * fresh observation.
 *
 * K11.1 — Decay/half-life on stale evidence. Both α and β decay toward the
 * Beta(2,2) prior over `halfLifeWeeks` weeks of inactivity. The decay applies
 * lazily on read AND just-in-time on write so a long-dormant scenario whose
 * verdict suddenly turns adverse demotes its tier instead of compounding old
 * favourable evidence into auto-apply tiers it no longer deserves. The prior
 * is a hard floor — decay never undershoots Beta(2,2).
 */
;(function () {
    ...
    const DEFAULT_HALF_LIFE_WEEKS = 16            // K11.1 default — slow forget per §8.4 "compounding trust"
    ...
    function _readHalfLifeWeeks() {
        try {
            const s = window.AesConductorTrustSettings
            const cached = s && typeof s.cached === "function" ? s.cached() : null
            const v = cached && cached.decay && cached.decay.halfLifeWeeks
            if (typeof v === "number" && isFinite(v) && v > 0) return v
        } catch (_) { /* noop */ }
        return DEFAULT_HALF_LIFE_WEEKS
    }

    /** K11.1 — pure decay helper. Pulls (α, β) toward Beta(2, 2) prior by
     *  factor `2 ** (-Δt / halfLifeMs)` applied to the *evidence* counts
     *  (α-prior and β-prior). Floors at the prior so decay never undershoots
     *  the un-informed posterior. Returns a NEW entry; does not mutate. */
    function _decayPosterior(entry, now, halfLifeWeeks) {
        if (!entry || typeof entry.lastAt !== "number" || entry.lastAt <= 0) return entry
        const hlw = (typeof halfLifeWeeks === "number" && halfLifeWeeks > 0) ? halfLifeWeeks : DEFAULT_HALF_LIFE_WEEKS
        const halfLifeMs = hlw * WEEK_MS
        const dt = Math.max(0, (now || Date.now()) - entry.lastAt)
        if (dt < WEEK_MS) return entry                                    // skip cheap path under one week
        const factor = Math.pow(2, -dt / halfLifeMs)
        const alphaEvidence = Math.max(0, entry.alpha - PRIOR_ALPHA) * factor
        const betaEvidence  = Math.max(0, entry.beta  - PRIOR_BETA)  * factor
        const next = Object.assign({}, entry)
        next.alpha = PRIOR_ALPHA + alphaEvidence
        next.beta  = PRIOR_BETA  + betaEvidence
        next.n     = (next.alpha - PRIOR_ALPHA) + (next.beta - PRIOR_BETA)
        next.tq    = next.alpha / (next.alpha + next.beta)
        next.lcb   = _lcb(next.alpha, next.beta)
        next.tier  = _tierFromLcb(next.lcb, entry.tier)                  // hysteresis still applies
        // lastAt unchanged — we want elapsed time to grow until next observation
        // resets the clock via record().
        return next
    }
    ...
    async function load(host) {
        ...
        const out = {}
        const now = Date.now()
        const hlw = _readHalfLifeWeeks()
        for (const k of Object.keys(blob)) out[k] = _decayPosterior(_normalize(blob[k]), now, hlw)
        return out
    }
    ...
    async function get(host, scenarioId) {
        ...
        const blob = await _read(_key(host))
        return _decayPosterior(_normalize(blob[scenarioId]), Date.now(), _readHalfLifeWeeks())
    }

    /** Update posterior with one observation. `favourable === null` is a
     *  no-op (the verdict isn't terminal yet). Applies K11.1 decay BEFORE
     *  incorporating new evidence so a long-dormant scenario's accumulated
     *  evidence mass shrinks first; otherwise a single late-arriving good
     *  outcome could keep an apply-auto tier alive on stale evidence. */
    async function record(host, scenarioId, favourable) {
        if (!scenarioId || favourable == null) return null
        const key = _key(host)
        if (!key) return null
        const blob = await _read(key)
        const cur = _decayPosterior(_normalize(blob[scenarioId]), Date.now(), _readHalfLifeWeeks())
    ...
    window.AesConductorTrustStore = {
        ...
        WEEK_MS, DEFAULT_HALF_LIFE_WEEKS
    }
})()
