...
(function () {
    ...
    // Default: skin on, compact density. Match defaults in options.js.
    root.dataset.aesSkin = "on";
    root.dataset.aesDensity = "compact";
    ...
    function pageKindFromPath(pathname) {
        ...
        if (/^\/app\/enterprise\/marketing/.test(pathname))    return "marketing";
        if (/^\/app\/enterprise\/settings/.test(pathname))     return "settings";
        if (/^\/app\/alliance/.test(pathname))                 return "alliance";
        if (/^\/action\/info\/flight/.test(pathname))          return "flight-info";
        if (/^\/action\/enterprise\/staff(Overview|Pilots)/.test(pathname)) return "staff";
    ...
    function applyDensity(density) {
        const value = density === "comfortable" ? "comfortable" : "compact";
