...
;(function () {
    ...
    async function _enrichWear(server, aircraftId, wearView) {
        const out = {
            ratio:                  null,
            ratioStatus:            null,
            condition:              null,
        ...
        const viewEntry = (wearView && wearView.byTail) ? wearView.byTail[aircraftId] : null
        if (viewEntry) {
            out.ratio           = viewEntry.ratio
            out.ratioStatus     = viewEntry.ratioStatus
            out.condition       = viewEntry.condition
            out.conditionStatus = viewEntry.conditionStatus
        } else if (_has("AesAfpMaintenanceStore")) {
            try {
    ...
    async function _enrichFleet(server, fleetRaw, typesByTypeId, ledger) {
        ...
        const wearView = (typeof window.AesView !== "undefined"
                          && typeof window.AesView.get === "function")
            ? window.AesView.get("fleet:wear-rollup")
            : null
        const out = []
        for (const a of fleetRaw.aircraft) {
            ...
            const wear   = await _enrichWear(server, a.aircraftId, wearView)
